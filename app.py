...
def main():
    ...
    if selected == "PDF截圖與AI文案":
        ...
        if options == "每頁商品數固定":
            ...
            st.text_area("給 ChatGPT 的 Prompt", placeholder="・請確保prompt中提及對「主題」、「標題」與「文案」的撰寫要求\n\n・prompt中無須指定文案格式，生成的內容將自動於表格中分割為多個欄位",height=286, value=st.session_state.user_input1, key='user_input_input1', on_change=update_user_input1)
            ...
            col1.text_area("對應的截圖寬度（px）", placeholder="未填則預設為完整PDF頁寬\n\n數量：寬度（用換行分隔）", height=120, value=st.session_state.width_map_str, key='width_map_str_input', on_change=update_width_map_str, help="選填")
            col2.text_area("給 ChatGPT 的 Prompt", placeholder="・請確保prompt中提及對「主題」、「標題」與「文案」的撰寫要求\n\n・prompt中無須指定文案格式，生成的內容將自動於表格中分割為多個欄位",height=370, value=st.session_state.user_input2, key='user_input_input2', on_change=update_user_input2)
