...
def main():
    ...
    if selected == "PDF截圖與AI文案":
        ...
        if options == "每頁商品數固定":
            ...
            st.text_area("給 ChatGPT 的 Prompt", placeholder="程式中已預設文案生成的格式，Prompt中無須再指定\n\n應包含對「主題」、「標題」與「文案」的要求",height=286, value=st.session_state.user_input1, key='user_input_input1', on_change=update_user_input1)
            ...
            col2.text_area("給 ChatGPT 的 Prompt", placeholder="程式中已預設文案生成的格式，Prompt中無須再指定\n\n應包含對「主題」、「標題」與「文案」的要求",height=370, value=st.session_state.user_input2, key='user_input_input2', on_change=update_user_input2)
